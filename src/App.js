...
class App extends Component {
  ...
  VerMensagem() {
    ...
    let NumeroAleatorio = Math.floor(Math.random() * this.mensagens.length)
    state.TextoMensagem = '"' + this.mensagens[NumeroAleatorio] + '"'
    this.setState(state)
    state.stage = false
  }
...

        {
          this.state.stage ?
            <div>
              ...
              <h1 className="h1text2">Escolha um dos biscoitos abaixo e veja sua mensagem</h1>
              <br /><br />
              <Cookie AcaoBtn={this.VerMensagem} />
            </div>

            :

            <div>
              <h1 className="h1text" >{this.state.TextoMensagem}</h1>
              <br /><br />
              <Restart AcaoBtn={this.Restart} />
            </div>
        }
